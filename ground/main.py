...
while(True):
    ...
    if (joystick_controls_arm): # send joystick commands to the arm 
        if(abs(j.get_axis(1)) < 0.1): 
            pser.write((ARM_ADDRESS, '0', '\x00', '\x00', '\x00')) # don't let drift affect the arm
        elif(j.get_axis(1) > 0):
            pser.write((ARM_ADDRESS, '2', '\x00', '\x00', '\x00'))
        else:
            pser.write((ARM_ADDRESS, '1', '\x00', '\x00', '\x00')) 
        ...
        '''
        forward_backward = int(j.get_axis(1) * 127 + 128)
        left_right = int(j.get_axis(0) * 127 + 128 )
        pser.write((DRIVE_ADDRESS, chr(left_right), chr(forward_backward), '\x00', '\x00'))
